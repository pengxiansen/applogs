package com.pengzhaopeng.mock.client;

import com.alibaba.fastjson.JSONObject;
import com.pengzhaopeng.comon.BasicLog;
...
public class GenBeahavior01 {
    ...
    private static String[] appId = {"app00001", "app00002", "app00003", "app00004", "app00005", "app00006", "app00007"};
    ...
    public static void gather() {
        ...
        try {
            for (int i = 1; i <= 200000000; i++) {
                ...
                StartupReportLogs appStartupLogs = initAppStartupLogs(BasicLog.STARTUP_REPORT_LOGS, userId, userPlatform, beginTime);

                PageVisitReportLogs pageVisitReportLogs = initPageVisitReportLogs(BasicLog.PAGEVISIT_REPORT_LOGS, userId, userPlatform, beginTime);

                ErrorReportLogs errorReportLogs = initErrorReportLogs(BasicLog.ERROR_REPORT_LOGS, userId, userPlatform, beginTime);


                String startupJson = JSONObject.toJSONString(appStartupLogs);
                UploadUtil.upload(startupJson, 0);

                String pageJson = JSONObject.toJSONString(pageVisitReportLogs);
                UploadUtil.upload(pageJson, 1);

                String errorJson = JSONObject.toJSONString(errorReportLogs);
                UploadUtil.upload(errorJson, 2);
     ...
     * @return
     */
    private static StartupReportLogs initAppStartupLogs(int type, String userId, String userPlatform, Long beginTime) {
        StartupReportLogs appStartupLog = new StartupReportLogs();

        appStartupLog.setType(type);
        appStartupLog.setUserId(userId);
        appStartupLog.setAppPlatform(userPlatform);

        appStartupLog.setAppId(appId[random.nextInt(appId.length)]);
     ...
     * @return
     */
    private static PageVisitReportLogs initPageVisitReportLogs(int type, String userId, String userPlatform, Long beginTime) {
        PageVisitReportLogs appPageReportLog = new PageVisitReportLogs();

        appPageReportLog.setType(type);
        appPageReportLog.setUserId(userId);
        appPageReportLog.setAppPlatform(userPlatform);
        appPageReportLog.setAppId(appId[random.nextInt(appId.length)]);
     ...
     * @return
     */
    private static ErrorReportLogs initErrorReportLogs(int type, String userId, String userPlatform, Long beginTime) {
        ErrorReportLogs errorReportLog = new ErrorReportLogs();

        errorReportLog.setType(type);
        errorReportLog.setUserId(userId);
        errorReportLog.setAppPlatform(userPlatform);
        errorReportLog.setAppId(appId[random.nextInt(appId.length)]);
